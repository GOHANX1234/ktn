...
// =========================================================================== //
#include <dlfcn.h>   // IMPORTANT

void Il2CppAttach(const char *name) {
    void *handle = dlopen(name, RTLD_NOW);   // replaced dlopen_ex

    while (!handle) {
        handle = dlopen(name, RTLD_NOW);     // replaced dlopen_ex
        sleep(1);
    }
    ...
    il2cpp_string_new_utf16 = (Il2CppString *(*)(const wchar_t *, int32_t)) dlsym(handle, "il2cpp_string_new_utf16");
    ...
    il2cpp_class_get_nested_types = (void *(*)(void *, void **)) dlsym(handle, "il2cpp_class_get_nested_types");

    dlclose(handle);   // replaced dlclose_ex
}
// =========================================================================== //
...
// ================================================================================================================ //
void *Il2CppGetClass(const char *image, const char *namespaze, const char *clazz) {
    string _sig = image;
    _sig += namespaze;
    _sig += clazz;
